...
* **デスクトップ領域の予約 (AppBar)**
  * アプリ起動時、各画面（モニター）の上部にタスクバー領域（高さ45px）をシステム予約し、他のアプリケーションが最大化された際もタスクバーと重ならないように制御します。
...
graph TD
    ...
    MainWindow -->|領域予約/移動ロック| AppBarHelper[AppBarHelper]
```
...
| コンポーネント名 | 役割・概要 |
| :--- | :--- |
| **MainWindow** | 各モニターに個別に描画され、タスクバーUIと対応カラムのリストボックスを表示します。 |
...
| **AppBarHelper** | アプリケーションの各ウィンドウをWindowsの「AppBar」として個別登録し、デスクトップ予約領域を各画面に確保するとともに、ウィンドウ移動・リサイズ命令を遮断して固定します。 |
...
* **シャットダウン順序の保証**: `MainWindow` の閉じる処理において、WPF の `Window_Closed` イベント発生時に `AppBarHelper.Unregister`（AppBarの登録解除）を呼び出す前に、必ず `WindowManager.Instance.Shutdown()` を実行します。また、クリーンアップ処理は `try-catch` で保護し、個別のエラーが全体の復元処理を阻害しないようにします。
...
### 4.6. マルチモニターのAppBar位置決めと移動防止ロック (AppBar Registry & Lock)
複数の `MainWindow` が起動されるため、`AppBarHelper` は `Dictionary<IntPtr, AppBarState>` を用いて各ウィンドウの登録状態を個別に保持します。
* **動的配置**: 登録時、各ウィンドウの HWND から属するモニター情報（`Screen.FromHandle`）を動的に引き当て、該当モニターの上部境界に対して正確に領域を予約・配置します。
* **OS移動コマンドの遮断**: `WM_WINDOWPOSCHANGING` メッセージフックにおいて、OS側の割り込みによる座標・サイズの変更要求を、予約されたモニターの絶対配置座標（`AppBarRect`）で強制的に上書きし、`Win + Shift + 矢印` 等によるタスクバー自体の意図しないモニター間移動を完全に防御します。
